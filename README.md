# Northern Azure User Group (NAUG) - Community Event Repository
Repo for all NAUG User group sessions, including speaker materials, recordings, and event resources.

Welcome to the **North Azure User Group (NAUG)** community event repository. This is the central hub for:
- 📅 Monthly meetup schedule and speaker information
- 🎤 Event presentations, slides, and recordings
- 📋 Community event playbook and operational checklists
- 🤝 Speaker and volunteer coordination

**Want to speak?** See [CONTRIBUTING.md](CONTRIBUTING.md) for our speaker submission process.

**Looking for details?** Check out [FAQ.md](FAQ.md) for common questions about attending and speaking.

---

## Quick Navigation

- [2026 Meetup Schedule](#2026-meetup-schedule)
- [Speaker Pipeline](#speaker-pipeline)
- [Event Run of Show](#event-run-of-show)
- [How to Get Involved](#how-to-get-involved)
- [FAQ](#faq)
- [Repository Structure](#repository-structure)
- [Community Guidelines](#community-guidelines--code-of-conduct)

---

## About Us

**Join the North Azure User Group (NAUG)** – Community-driven cloud learning for the North of England.

📍 **Meetup:** [northern-azure-user-group](https://www.meetup.com/northern-azure-user-group)

### Our Mission

The aim of NAUG is to bring the IT community within Leeds, Manchester and surrounding areas together. The group is community-driven, delivering the type of content, speakers, and experiences that the IT community wants. We're open to developers, architects, consultants, engineers, and enthusiasts.

### What You'll Gain

- Learn best practices and optimal use of Microsoft's Cloud platform
- Get answers, advice, tips, and suggestions from peers
- Gain insights into what others are doing within Azure
- Network with local peers

---

## 2026 Meetup Schedule

| Month | Date | Venue | Speakers / Talks |
|-------|------|-------|-----------------|
| ~~January~~ | | | |
| ~~February~~ | | | |
| ~~March~~ | | | |
| ~~April~~ | | | |
| May | 19/05/2026 | Bonded Warehouse | Elliott Leighton-Woodruff - Drift Happens: Intelligent Ways to Detect and Remediate It<br>Michael Royster - Introducing the Github Copilot CLI |
| June | 13/06/2026 | ANS Archway | Microsoft Build //Localhost:Manchester |
| June | 23/06/2026 | Bonded Warehouse | TBC |
| July | 21/07/2026 | Bonded Warehouse | TBC |
| August | TBC | Bonded Warehouse | TBC |
| September | TBC | Bonded Warehouse | TBC |
| October | TBC | Bonded Warehouse | TBC |
| November | TBC | Bonded Warehouse | TBC |
| December | TBC | TBC | 🎄 Christmas Event |

---

## Community Event Playbook

### Speaker Pipeline

- **Call for Proposals**: Send CFP 4 weeks before event
- **Speaker Requirements**:
  - Title and abstract (200 words max)
  - Speaker bio and headshot
  - LinkedIn/social links
  - Tech/AV requirements
  - Consent to record and share recording
- **Slide Deadline**: 1 week before event

### Event Run of Show

**Typical format:**

| Time | Duration | Segment | Owner |
|------|----------|---------|-------|
| 18:00 | 30m | Arrival & Networking | Host |
| 18:30 | 5m | Welcome & Housekeeping | Host |
| 18:35 | 45m | Talk 1 | Speaker 1 |
| 19:20 | 30m | 🍕 Break & Networking | Host |
| 19:50 | 45m | Talk 2 | Speaker 2 |
| 20:35 | 5m | Wrap up & Q&A | Host |

### Repository Structure

```
NAUG-Sessions/
├── README.md
├── CODE_OF_CONDUCT.md
├── CONTRIBUTING.md
├── FAQ.md
├── LICENSE
├── assets/
│   ├── logos/             # NAUG logos and branding
│   └── slide-templates/   # Slide templates for speakers
├── events/
│   ├── 2026/              # Current year events
│   │   ├── May/
│   │   │   ├── event-summary.md
│   │   │   └── slides/
│   │   ├── June/
│   │   └── ...
│   ├── 2025/              # Past events
│   │   ├── May/
│   │   └── ...
│   └── archive/           # Events prior to 2025
│       ├── 2024/
│       ├── 2023/
│       └── ...
```

---

## How to Get Involved

### 🎤 Propose a Talk
Interested in speaking? Get in touch via our [Meetup page](https://www.meetup.com/northern-azure-user-group) with your talk title, abstract, bio, and availability. See [CONTRIBUTING.md](CONTRIBUTING.md) for details.

### 💬 Share Feedback
Found a venue we should know about? Have ideas for future topics? Open an issue on GitHub or email us.

### 📖 Contribute to This Repository
Help us improve documentation or add event details. See [CONTRIBUTING.md](CONTRIBUTING.md) for guidelines.

---

## FAQ

**Common questions?** Check [FAQ.md](FAQ.md) for answers about attending, speaking, and more.

---

## Community Guidelines & Code of Conduct

📋 **See [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md) for our full Community Guidelines and Code of Conduct.**

---

## License

This repository is licensed under [CC BY-NC 4.0](LICENSE). You are free to view and contribute, but the content may not be reused for commercial purposes. See [LICENSE](LICENSE) for full details.